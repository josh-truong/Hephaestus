...
class SpeedController():
    ...
    def autonomous(self, velocity_rate):
        ...
        if (waypoints is None or waypoints == []):
            self.w.env.rerun_rrt = True
            return 0, 0

        goal = waypoints[state]
        self.w.env.goal = Pose(goal[0],goal[1])
        ...
        if (rho < self.flex_rtol):
            ...
            self.w.robot.msg = "Moving to next state."
            if (state == len(waypoints)-1):
                state = 0
                self.w.env.num_completed_paths += 1
                self.w.env.rerun_rrt = True
                self.w.robot.msg = "Path reached."
        ...
        if (self.r.debug):
            ...
            print(f"       current: ({pose.x:< .2f}, {pose.y:< .2f}, {pose.theta:< .2f})")
            print(f"[{state:<4}] goal:    ({goal.x:< .2f}, {goal.y:< .2f}, {goal.theta:< .2f})")
            print(f"[{len(waypoints)-1:<4}] final:   ({waypoints[-1][0]:< .2f}, {waypoints[-1][1]:< .2f})")
            print(f"# of viable waypoints: {len(self.r.env.vaiable_waypoints)}")
            print(f"Current Map Bound: [{self.r.env.xmax_boundary}, {self.r.env.ymax_boundary}]")
            print(f"Completed paths: {self.r.env.num_completed_paths} / {self.r.env.max_completed_paths}")
